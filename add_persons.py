...
def add_to_database(ci):
    """
    Add a person's name and surname to the face_recognition_db database.
    ...
    db = mysql.connector.connect(
        ...
        database="face_recognition_db" # Your database name
    )

    cursor = db.cursor()

    # SQL query to insert a new participant
    sql = "INSERT INTO participants (ci) VALUES (%s)"
    val = (ci,)
...
def add_persons(backup_dir, add_persons_dir, faces_save_dir, features_path):
    ...
    for sub_dir in os.listdir(add_persons_dir):
        ...
        folder_name = os.path.basename(dir_to_move)
        add_to_database(folder_name)
